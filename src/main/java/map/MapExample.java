...
//        processUnmodifiableMap();
    ...
    public static void processMap() {
        ...
        map.putIfAbsent("thirty", 300);  // it won't put, because "thirty" already is present
        ...
        System.out.println("New value for \"eighty\": " + map.merge("eight", 80, biFunction));
        System.out.println("New value for \"eighty\": " + map.merge("eight", 80, biFunction));
    ...
    public static void replaceAndRemove() {
        ...
        BiFunction<String, Integer, Integer> replacementFunction = (k, v) -> k.length() + v;
        map.replaceAll(replacementFunction);
    ...
    public static void computeFunctions() {
        ...
        map.compute("one", remappingFunction1);  // NullPointerException if the key is not present
        entrySet.forEach(System.out::println);
        ...
        entrySet.forEach(System.out::println);
        map.computeIfPresent("elevenx", remappingFunction2);  // it won't map because "elevenx" does not resent
        entrySet.forEach(System.out::println);
    }

    public static void processUnmodifiableMap() {
        Map map = Map.of(1, "Jon", 2, "Elizabet", 3, "Charles", 4, "Mark");
        System.out.println("\nKeys and Values:");
        Set keys = map.keySet();
        for (Object key : keys) {
            Object value = map.get(key);
...
//        map.put(5, "Enrique");  // We can't change it
    }
}
